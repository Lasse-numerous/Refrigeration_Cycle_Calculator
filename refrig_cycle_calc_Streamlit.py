...
def main():
    ...
    with st.sidebar:
        ...
        # Evaporator
        evap_type = st.selectbox(
            "Evaporator Input Type:",
            ["Temperature (°F)", "Pressure (psia)"]
        )
        evap_value = st.number_input(
            "Evaporator Value:",
        ...
        # Other parameters
        superheat_F = st.number_input(
            "Superheat (°F):",
            min_value=0.0,
            max_value=30.0,
            value=10.0
        )
        subcooling_F = st.number_input(
            "Subcooling (°F):",
            min_value=0.0,
            max_value=30.0,
            value=10.0
        )
        isentropic_eff = st.number_input(
            "Compressor Isentropic Efficiency (%):",
            min_value=20.0,
            max_value=100.0,
            value=70.0
        )
        mass_flow_lb_min = st.number_input(
    ...
    if st.button("Calculate"):
        try:
            ...
            # --- Validation: Condenser must be greater than Evaporator ---
            if sat_cond_T <= sat_evap_T:
                st.error("Condenser temperature must be greater than evaporator temperature.")
                return
            if high_pressure <= low_pressure:
                st.error("Condenser pressure must be greater than evaporator pressure.")
                return
